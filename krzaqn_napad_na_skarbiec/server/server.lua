...
RegisterServerEvent('krzaqn_napad_na_skarbiec:zadaleko')
AddEventHandler('krzaqn_napad_na_skarbiec:zadaleko', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('krzaqn_napad_na_skarbiec:usunblipa', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('krzaqn_napad_na_skarbiec:zadalekolocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('krzaqn_napad_na_skarbiec:napad')
AddEventHandler('krzaqn_napad_na_skarbiec:napad', function(robb)
	...
	if Banks[robb] then
		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('wiertlo').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('krzaqn_napad_na_skarbiec:ustawblipa', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('krzaqn_napad_na_skarbiec:aktualnienapadany', source, robb)
				...
				SetTimeout(60, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('krzaqn_napad_na_skarbiec:napadzakonczony', savedSource, job)
						if(xPlayer)then

							xPlayer.addInventoryItem('goldznapadu', bank.reward)
							local xPlayers = ESX.GetPlayers()
							for i=1, #xPlayers, 1 do
								local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
								if xPlayer.job.name == 'police' then
