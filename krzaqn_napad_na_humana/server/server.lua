...
RegisterServerEvent('krzaqn_napad_na_humana:zadaleko')
AddEventHandler('krzaqn_napad_na_humana:zadaleko', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('krzaqn_napad_na_humana:usunblipa', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('krzaqn_napad_na_humana:zadalekolocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('krzaqn_napad_na_humana:napad')
AddEventHandler('krzaqn_napad_na_humana:napad', function(robb)
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('krzaqn_napad_na_humana:ustawblipa', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('krzaqn_napad_na_humana:aktualnienapadane', source, robb)
				...
				SetTimeout(100, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('krzaqn_napad_na_humana:napadzakonczony', savedSource, job)
						if(xPlayer)then

							xPlayer.addInventoryItem('daneznapadu', bank.reward)
							local xPlayers = ESX.GetPlayers()
							for i=1, #xPlayers, 1 do
								local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
								if xPlayer.job.name == 'police' then
