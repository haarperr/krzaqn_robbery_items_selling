...
RegisterServerEvent('krzaqn_napad_na_banki:toofar')
AddEventHandler('krzaqn_napad_na_banki:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('krzaqn_napad_na_banki:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('krzaqn_napad_na_banki:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('krzaqn_napad_na_banki:rob')
AddEventHandler('krzaqn_napad_na_banki:rob', function(robb)
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('krzaqn_napad_na_banki:setblip', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('krzaqn_napad_na_banki:currentlyrobbing', source, robb)
				...
				SetTimeout(600000, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('krzaqn_napad_na_banki:robberycomplete', savedSource, job)
						if(xPlayer)then
